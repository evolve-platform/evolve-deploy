...
// What a release owns is the image and the environment. Everything else on the
// container is Terraform's, and a plan that carried the running values over
// staged a just-applied change straight back out — a liveness probe kept
// polling the port the service used to listen on, and every instance was killed
// for failing it.
func TestAdoptRuntimeKeepsTheDeclaredContainer(t *testing.T) {
	declared := []*runpb.Container{{
		Name:  "app",
		Image: "eu.gcr.io/p/site:bootstrap",
		...
		Env: []*runpb.EnvVar{
			{Name: "STALE", Values: &runpb.EnvVar_Value{Value: "from-the-first-create"}},
		},
	}}
	running := []*runpb.Container{{
		Name:  "app",
		Image: "eu.gcr.io/p/site:2daef25",
		Ports: []*runpb.ContainerPort{{Name: "http1", ContainerPort: 3000}},
		LivenessProbe: &runpb.Probe{
			PeriodSeconds: 10,
			ProbeType: &runpb.Probe_HttpGet{
				HttpGet: &runpb.HTTPGetAction{Path: "/api/healthcheck", Port: 3000},
			},
		},
		Env: []*runpb.EnvVar{
			{Name: "REDIS_URL", Values: &runpb.EnvVar_Value{Value: "redis://live"}},
		},
	}}

	adoptRuntime(declared, running)

	got := declared[0]
	if got.GetImage() != "eu.gcr.io/p/site:2daef25" {
		t.Errorf("Image = %q, want the one that is running", got.GetImage())
	}
	if len(got.GetEnv()) != 1 || got.GetEnv()[0].GetName() != "REDIS_URL" {
		t.Errorf("Env = %v, want the environment that is running", got.GetEnv())
	}
	if port := got.GetPorts()[0].GetContainerPort(); port != 4000 {
		t.Errorf("ContainerPort = %d, want the declared 4000", port)
	}
	probe := got.GetLivenessProbe()
	if probe.GetPeriodSeconds() != 60 || probe.GetHttpGet().GetPort() != 4000 {
		t.Errorf("liveness probe = %v, want the declared one", probe)
	}
	if got.GetEnv()[0] == running[0].GetEnv()[0] {
		t.Error("the environment is shared with the revision rather than copied")
	}
}

// A container the revision has never seen is one Terraform has just added, so
// there is nothing running to adopt and what it declares has to stand.
func TestAdoptRuntimeLeavesANewSidecarAlone(t *testing.T) {
	declared := []*runpb.Container{
		{Name: "app", Image: "eu.gcr.io/p/site:bootstrap"},
		{Name: "collector", Image: "otel/collector:2"},
	}
	running := []*runpb.Container{{Name: "app", Image: "eu.gcr.io/p/site:2daef25"}}

	adoptRuntime(declared, running)

	if declared[1].GetImage() != "otel/collector:2" {
		t.Errorf("sidecar Image = %q, want the declared one", declared[1].GetImage())
	}
}
